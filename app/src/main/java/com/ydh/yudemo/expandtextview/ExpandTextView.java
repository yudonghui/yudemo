...
public class ExpandTextView extends TextView implements View.OnClickListener {
    ...
    public ExpandTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        arrowDrawablePadding = (int) typedArray.getDimension(R.styleable.ExpandTextView_arrowPadding, DensityUtil.dp2px(2));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //测量TextView总高度
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        if (getLineCount() <= mMaxCollapsedLines) {//当前文字行数小于等于最大行数。那么就不可以展示
            needCollapse = false;
            return;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (needCollapse) {
            int left, top;
            if (arrowPosition == POSITION_RIGHT) {
                left = getWidth() - getTotalPaddingRight() + arrowDrawablePadding;//图片离左边的距离。
                switch (arrowAlign) {
                    case ALIGN_LEFT_TOP:
    ...
    private int getRealTextViewHeight(TextView textView) {
        int textHeight = textView.getLayout().getLineTop(textView.getLineCount());//获取文字的高度
        /*
        * 这里用getCompoundPaddingTop 而不是getPaddingTop
        * 主要是因为有图片。getPaddingTop是整个textview控件的内边距
        * getCompoundPaddingTop是文字顶部距离控件顶部的距离
        * */
        int padding = textView.getCompoundPaddingTop() + textView.getCompoundPaddingBottom();//获取文字上下padding
